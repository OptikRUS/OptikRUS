...
Старший бэкенд-разработчик с опытом 6+ лет в проектировании высоконагруженных распределённых систем на Python. Сократил задержку API **в 36 раз** (с 3500 мс до 95 мс) на таблицах в 20M строк. <!-- [Evidence: knowledge_base/compel/search_optimization_deep_dive.md] -->

Спроектировал событийно-ориентированную платформу, обрабатывающую **16–19M событий Kafka** с нулевым простоем благодаря стратегии двойного пайплайна и feature flags. <!-- [Evidence: knowledge_base/compel/kafka_pipeline_deep_dive.md] --> Стандартизировал шаблоны микросервисов, **сократив время развёртывания вдвое**. <!-- [Evidence: knowledge_base/compel/microservice_template.md] -->

Драйвер роста команды: наставлял 7+ специалистов, из которых 4+ вышли на продвинутый уровень или получили повышение; внедрил культуру TDD/BDD и **сократил количество дефектов на 70%**. <!-- [Evidence: knowledge_base/compel/bdd_process.md; knowledge_base/fortech/mentoring_and_leadership.md] -->

Практикую **AI-native разработку** — TDD-процессы на базе Claude Code с автоматическим AI-ревью кода и промпт-инженерией для BDD-сценариев.
...
- **Нагрузочное тестирование**: провёл нагрузочное тестирование с **Locust**, выявил узкие места и сократил **время отклика в 30 раз** на критических API-методах <!-- [Evidence: knowledge_base/compel/search_optimization_deep_dive.md] -->
- **Событийный пайплайн на Kafka**: спроектировал пайплайн на **Kafka**, обрабатывающий **16–19M событий** с синхронизацией с минимальной задержкой и нулевым простоем благодаря стратегии двойного пайплайна и feature flags <!-- [Evidence: knowledge_base/compel/kafka_pipeline_deep_dive.md] -->
...
- **Внедрение BDD-процесса**: внедрил **BDD-сценарии** и обучил системных аналитиков их написанию, повысив качество требований <!-- [Evidence: knowledge_base/compel/bdd_process.md] -->
...
- **Техническое наставничество**: обучил **3 системных аналитиков** BDD-практикам и системной архитектуре <!-- [Evidence: knowledge_base/compel/bdd_process.md] -->
- **AI-native процесс разработки**: интегрировал **Claude Code** в ежедневный цикл TDD (Red-Green-Refactor) с кастомными навыками, субагентами и плагинами для генерации тестов, рефакторинга и автоматического ревью кода <!-- [Evidence: knowledge_base/compel/ai_native_development.md] -->
- **Промпт-инженерия**: применил **NotebookLM** и структурированные промпты для генерации BDD-сценариев, документации и автоматизации тестов с сохранением доменной точности <!-- [Evidence: knowledge_base/compel/ai_native_development.md] -->
...
- **CI/CD-конвейеры**: настроил конвейеры с тестами, линтерами и проверками типов, **сократив релизный цикл на 40%** и минимизировав регрессии в продакшене <!-- [Evidence: knowledge_base/fortech/ci_cd_quality.md] -->
- **Наставничество и адаптация**: наставлял новых инженеров, ускоряя их адаптацию и выход на продуктивность; проводил кросс-командные ревью кода для раннего выявления архитектурных ошибок <!-- [Evidence: knowledge_base/fortech/mentoring_and_leadership.md] -->
- **Техническое лидерство**: проводил технические собеседования и ревью производительности, выявлял зоны роста; выполнял системный анализ сторонних интеграций, писал документацию и обучал разработчиков
...
- **Наставничество junior-разработчиков**: наставлял 2 junior-разработчиков, доведя их до уровня самостоятельных специалистов <!-- [Evidence: knowledge_base/fortech/mentoring_and_leadership.md] -->
- **Интеграция LLM и популяризация AI**: интегрировал **ChatGPT API** в Telegram-бот для автоматизированной подготовки алгоритмов к собеседованиям; провёл общекорпоративную презентацию по практическому применению ИИ для инженерных команд во всех офисах
...
- **CI/CD-инфраструктура**: развернул GitLab CI с Docker, автоматизировав полный конвейер сборки, тестирования и развёртывания
...
- **Защита проекта**: успешно защитил проект перед заинтересованными сторонами с архитектурным обоснованием и демонстрацией надёжности в реальном времени
...
- **Сервис ипотечного калькулятора**: спроектировал и запустил в продакшен сервис ипотечного расчёта с нуля за **3–4 недели**, изолировав доменную логику через FastAPI + Django Admin с feature flags <!-- [Evidence: knowledge_base/artw/mortgage_service_details.md] -->
...
- **Внедрение тестового покрытия**: добился повышения покрытия тестами **с 0% до 50%**
- **Техническое лидерство**: взял на себя обязанности тимлида — планирование спринтов, декомпозиция фич, управление релизами
- **Рефакторинг устаревшего кода**: провёл рефакторинг устаревшей кодовой базы, упростив дальнейшую поддержку системы
- **Миграция данных без потерь**: управлял миграцией данных между сервисами без потерь
- **Ревью кода и улучшение качества**: проводил кросс-командные ревью кода; внедрил линтеры перед уходом
...
**Проект**: IoT-дашборд — метрики устройств с минимальной задержкой

- **Сервис-потребитель Kafka**: построил сервис-потребитель Kafka (один консьюмер, ~5 сообщений/сек) с кэшем в памяти для отображения последнего состояния, отдавая приоритет низкой задержке над надёжностью
...
- **CI/CD-автоматизация**: развернул CI/CD-конвейер с Docker и GitHub Actions для автоматизированных сборок и релизов
...
## 🤝 Гибкие навыки
- **Драйвер роста команды**: наставлял 7+ специалистов разного уровня через структурированное обучение TDD/BDD, архитектурные практикумы и инженерию требований
- **Опыт наставничества**:
  - **Компэл**: обучил 3 системных аналитиков ERD, Swagger и BDD-практикам, повысив качество требований
  - **Fortech**: наставлял 2 junior-разработчиков до уровня самостоятельных специалистов
  - **OTR**: перевёл команду из 3 человек с передачи zip-архивов на GitHub и внедрил кросс-командное ревью кода
- Быстро осваиваю новые технологические стеки — выпускал продуктовые фичи на незнакомых фреймворках уже в первый спринт
- **Совершенствование Agile-процессов** (Scrum, Kanban): улучшал декомпозицию задач, планирование спринтов и предсказуемость сроков
- Доступно объясняю сложные архитектурные решения нетехническим заинтересованным сторонам — успешно защищал проекты перед руководством
...
5-кратный финалист хакатона «Лидеры цифровой трансформации».
</p>

***
<div align="center">

[![Typing SVG](https://readme-typing-svg.herokuapp.com?color=%23000202&size=25&multiline=true&width=640&lines=Информация+обновлена+в+апреле+2026;.+.+.)](#кирилл-медведко)
