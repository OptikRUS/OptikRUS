...
Старший бэкенд-разработчик с 6+ годами опыта построения высоконагруженных распределённых систем на Python. Добился **снижения задержки в 36 раз** (с 3500 мс до 95 мс) на наборах данных в 20M строк <!-- [Evidence: knowledge_base/compel/search_optimization_deep_dive.md] -->, спроектировал событийно-ориентированную платформу, обрабатывающую **16–19M событий Kafka** с нулевым простоем через стратегию двойного пайплайна и флаги функциональности <!-- [Evidence: knowledge_base/compel/kafka_pipeline_deep_dive.md] -->, и стандартизировал шаблоны микросервисов, **сократив время развёртывания вдвое** <!-- [Evidence: knowledge_base/compel/microservice_template.md] -->. Мультипликатор эффективности: наставил 4+ инженеров до уровня повышения, внедрил культуру TDD/BDD, **сократив количество багов на 70%** <!-- [Evidence: knowledge_base/compel/bdd_process.md] -->.
...
- **Нагрузочное тестирование**: провёл нагрузочное тестирование с **Locust**, выявил узкие места и добился **улучшения времени отклика в 30 раз** на критических эндпоинтах <!-- [Evidence: knowledge_base/compel/search_optimization_deep_dive.md] -->
- **Событийный пайплайн на Kafka**: спроектировал пайплайн на **Kafka**, обрабатывающий **16–19M событий** с синхронизацией в режиме, близком к реальному времени, и нулевым простоем через стратегию двойного пайплайна и флаги функциональности <!-- [Evidence: knowledge_base/compel/kafka_pipeline_deep_dive.md] -->
- **Асинхронный экспорт из MinIO**: реализовал асинхронный экспорт данных из **MinIO**, устранив ручные процессы и снизив нагрузку на основную БД
- **Автоматизация тестирования и качество**: внедрил **pytest**, доведя покрытие кода **до 98%** и **сократив баги в продакшене на 70%** через BDD-практики <!-- [Evidence: knowledge_base/compel/bdd_process.md] -->
...
- **Восстановление качества CI/CD**: получил согласие команды на восстановление отключённых линтеров и проверок типов в одном merge request, установив единый стиль кода и **устранив споры о форматировании** при ревью <!-- [Evidence: knowledge_base/fortech/ci_cd_quality.md] -->
...
- **Наставничество и онбординг**: наставлял и адаптировал новых инженеров, ускоряя их выход на продуктивность; проводил кросс-командные ревью кода для раннего выявления архитектурных ошибок <!-- [Evidence: knowledge_base/fortech/mentoring_and_leadership.md] -->
...
- **Внешнее наставничество**: коммерчески наставлял 2 junior-разработчиков вне работы, которые выросли до самостоятельных специалистов <!-- [Evidence: knowledge_base/fortech/mentoring_and_leadership.md] -->
...
- **Интеграция OpenCV**: интегрировал алгоритм распознавания на основе OpenCV в систему видеонаблюдения, улучшив скорость распознавания **на 40%** и точность **на 30%**
...
- **Декомпозиция задач и планирование спринтов**: в роли и.о. тимлида обеспечивал сбалансированную нагрузку и чёткие требования
- **Архитектура мобильного бэкенда**: спроектировал масштабируемую и отказоустойчивую архитектуру (FastAPI + Django Admin с расширенным UI + TortoiseORM)
- **REST API с RBAC**: разработал высокопроизводительный REST API с ролевым управлением доступом
...
- **Стабилизация интеграции AmoCRM**: стабилизировал интеграцию со сторонними сервисами (AmoCRM, DvizhAPI) через **ограничитель частоты на семафорах** (asyncio Semaphore), **устранив ошибки 429** и паттерны непреднамеренной самоDDoS-атаки <!-- [Evidence: knowledge_base/artw/self_ddos_details.md] -->
- **Исправление консистентности данных**: исправил критический баг консистентности данных, переупорядочив последовательность API-вызовов и записи в БД, обеспечив атомарность операций и устранив осиротевшие записи <!-- [Evidence: knowledge_base/artw/self_ddos_details.md] -->
- **Продвижение тестового покрытия**: добился повышения покрытия тестами **с 0% до 50%**, значительно снизив дефекты в продакшене
- **Обязанности тимлида**: взял на себя обязанности тимлида — планирование спринтов, декомпозиция фич, управление релизами
...
- **Read-through кэш для Dadata API**: реализовал кэш сквозного чтения для Dadata API через PostgreSQL с нормализованными ключами запросов и B-tree индексами, **устранив все баны от превышения лимитов** — система оставалась стабильной даже под нагрузочным тестированием <!-- [Evidence: knowledge_base/otr/dadata_cache_details.md] -->
...
- **Осознанный выбор технологии**: выбрал PostgreSQL вместо Redis для кэширования — адекватное решение для нагрузки ~100–500 запросов/день с осознанной простотой <!-- [Evidence: knowledge_base/otr/dadata_cache_details.md] -->
...
## 🤝 Гибкие навыки:
- **Мультипликатор эффективности команды**: наставил 4+ инженеров до уровня повышения через структурированное обучение TDD/BDD, архитектурные воркшопы и инженерию требований
- **Трек наставничества**:
...
- Быстро осваиваю новые технологические стеки — демонстрировал выпуск продуктовых фич на незнакомых фреймворках уже в первый спринт
- **Улучшение Agile-процессов** (Scrum, Kanban): измеримо сокращал время выполнения задач и повышал предсказуемость спринтов **на 35%**
- Доношу сложные архитектурные решения до нетехнических стейкхолдеров — успешно защищал проекты перед руководством
- Быстро анализирую крупные системы, находя узкие места и оптимизируя критические пути (ускорение задержки в 36 раз, ускорение импорта в 12 раз)
...
Принёс измеримый бизнес-результат в B2B-маркетплейсах, платформах лояльности, платформах недвижимости и государственных системах.
Подтверждённый опыт трансформации инженерной культуры через наставничество, разработку через тестирование и архитектурные стандарты —
последовательно повышаю эффективность команд. 4-кратный финалист хакатона «Лидеры цифровой трансформации».
</p>
